...
export function getInfoPokemonSuccess({id, name, weight, height, base_experience, types, abilities}: PokemonInfoProps) {

  /**
   * Приводим результаты к нужному виду
   */
  const abilitiesFormat = abilities.map(({ability: {name: nameAbility, url}}) => {
